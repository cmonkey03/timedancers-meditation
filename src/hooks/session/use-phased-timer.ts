...
export function usePhasedTimer(initialPhases: Phase[]) {
  const [timerState, setTimerState] = useState<TimerState>(() => Timer.newState(initialPhases));
  ...
  const setPhases = useCallback((next: Phase[]) => {
    setTimerState((prev) => {
